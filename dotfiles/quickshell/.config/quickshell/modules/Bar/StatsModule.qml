...
BaseModule {
    ...
    Process {
        id: gpuProcess
        stdout: StdioCollector {
            onStreamFinished: parseGpuStats(text.trim())
        }
        stderr: StdioCollector {
            onStreamFinished: if (text.trim() && !gpuAvailable)
                gpuStatus = text.trim()
        }
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                var updated = [];
                var changed = false;
                for (var j = 0; j < gpuApps.length; j++) {
                    var app = gpuApps[j];
                    var resolved = map[String(app.pid)];
                    if (resolved && resolved !== app.name) {
                        changed = true;
                        updated.push({
                            name: resolved,
                            pid: app.pid,
    ...
    function resolveGpuAppNames() {
        ...
        if (pids.length === 0)
            return;
        var cmd = "for p in " + pids.join(" ") + "; do exe=$(readlink \"/proc/$p/exe\" 2>/dev/null); name=${exe##*/}; name=${name#.}; echo \"$p\t$name\"; done";
        gpuNameProcess.exec({
            command: ["sh", "-c", cmd]
        });
    }
    ...
    function parseAmdGpuStats(jsonText) {
        try {
            ...
            if (!data.devices || data.devices.length === 0)
                throw new Error("amdgpu_top found no AMD devices");
            ...
            for (var pid in fdinfo) {
                ...
                if (gfx <= 0 && media <= 0 && mem <= 0)
                    continue;
                apps.push({
                    name: entry.name || pid,
                    pid: pid,
                    usage: Math.max(gfx, media),
                    memoryMb: mem
                });
            }
            apps.sort(function (a, b) {
                return (b.usage - a.usage) || (b.memoryMb - a.memoryMb);
            });
    ...
    function collectIntelClients(value, clients) {
        ...
        if (value.name !== undefined && value.pid !== undefined) {
            ...
            var usage = 0;
            for (var i = 0; i < busyValues.length; i++)
                usage = Math.max(usage, busyValues[i]);
            var memBytes = 0;
            ...
            clients.push({
                ...
                usage: usage,
                memoryMb: memBytes / 1048576
            });
            return;
        }
        for (var key in value)
            collectIntelClients(value[key], clients);
    }
    ...
    function parseIntelGpuStats(output) {
        ...
        try {
            ...
            gpuApps = clients.filter(function (app) {
                return app.usage > 0 || app.memoryMb > 0;
            }).sort(function (a, b) {
                return (b.usage - a.usage) || (b.memoryMb - a.memoryMb);
            }).slice(0, 5);
            resolveGpuAppNames();
    ...
    function intelGpuJsonText(text) {
        ...
        if (output.charAt(0) === "[") {
            ...
            if (innerStart < 0)
                throw new Error("intel_gpu_top returned empty array");
    ...
    function parseNvidiaGpuSnapshot(output) {
        ...
        if (parts.length < 5)
            return {
                available: false,
                status: "nvidia-smi returned incomplete GPU data"
            };
    ...
    function parseIntelGpuSnapshot(output) {
        ...
        try {
            ...
            return {
                ...
                apps: clients.filter(function (app) {
                    return app.usage > 0;
                }).slice(0, 5)
            };
        } catch (err) {
            return {
                available: false,
                status: "Failed to parse intel_gpu_top output: " + err
            };
        }
    }
    ...
    text: {
        ...
        if (gpuAvailable)
            parts.push("󰢮 " + Math.round(gpuUsage) + "%");
